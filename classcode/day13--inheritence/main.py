...
<<<<<<< HEAD
        ...
        self.spawners = [TieSpawner(2000, self.ships, self.bounds), YWingSpawner(2000,self.ships,self.bounds)]
    
    def handle_event(self, event):
        if event.type == MOUSEBUTTONDOWN and event.button == 1:
            self.xplos.add( Explosion(pygame.
        
        

    def update(self):
        dt = min(self.min_dt, self.clock.get_time()) #min frame rate
